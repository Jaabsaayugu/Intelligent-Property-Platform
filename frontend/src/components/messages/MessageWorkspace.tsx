...
export default function MessageWorkspace({
  ...
  const layoutClass = compact
    ? "grid gap-6 xl:grid-cols-[0.42fr_0.58fr]"
    : "grid gap-6 lg:grid-cols-[0.4fr_0.6fr]";
  ...
  const loadConversations = async () => {
    ...
    try {
      ...
      if (!selectedUserId && nextConversations[0]) {
        ...
        const otherUser = first.senderId === user?.id ? first.receiver : first.sender;
        setSelectedUserId(otherUser.id);
        setSelectedPropertyId(first.propertyId ?? null);
      }
    } catch (error) {
  ...
  const selectedConversation = useMemo(() => {
    return conversations.find((conversation) => {
      ...
      return (
        otherUser.id === selectedUserId &&
        (conversation.propertyId ?? null) === (selectedPropertyId ?? null)
      );
    });
  }, [conversations, selectedPropertyId, selectedUserId, user?.id]);
  ...
  if (!isAuthenticated) {
    return (
      <div className="rounded-[2rem] border border-slate-200 bg-white p-8 text-center text-slate-700">
  ...
  return (
    <div className={layoutClass}>
      <section className="hero-panel rounded-[2rem] border border-white/60 p-6 shadow-[0_24px_80px_-45px_rgba(15,23,42,0.55)] sm:p-8">
        ...
        <h2 className="mt-3 font-display text-3xl text-slate-900">{heading}</h2>
      ...
      <section className="hero-panel rounded-[2rem] border border-white/60 p-6 shadow-[0_24px_80px_-45px_rgba(15,23,42,0.55)] sm:p-8">
        ...
        {selectedConversation?.property?.title && (
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Property: <span className="font-semibold text-slate-900">{selectedConversation.property.title}</span>
          </p>
        )}
            ...
            return (
              <div
                ...
                className={`rounded-[1.5rem] px-4 py-3 text-sm leading-6 ${
                  isMine
                    ? "ml-10 bg-[#0f2747] text-white"
                    : "mr-10 bg-white/80 text-slate-700"
                }`}
              >
                <p className="font-semibold">
                  {isMine ? "You" : getDisplayName(message.sender, "User")}
                </p>
        ...
        {selectedUserId && (
          <form onSubmit={handleSend} className="mt-8 space-y-4">
            ...
            />
            <button className="rounded-full bg-[#0f2747] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#0b1d35]">
              Send reply
            </button>
          </form>
        )}
